...
int main(int argc, char *argv[]) 
{
  ...
  std::string str_points_filepath = "shuffledfeats.dat";
  std::string str_labels_filepath = "shuffledlabels.dat";

  // FILE* data_file;
  // FILE* label_file;

  // data_file = fopen(str_points_filepath.c_str(), "r");
  auto data_file  = readfile_vfs_char(str_points_filepath);
  std::string data_file_str(data_file.begin(), data_file.end());
  std::istringstream data_file_stream(data_file_str.c_str());
  ...
  {
    // TODO: here is very slow. Can we replace std::getline() with anything faster?
    // float tmp;
    std::string tmp;
    std::getline(data_file_stream, tmp);
    data_points[i] = std::stof(tmp);
  }
  // fclose(data_file);

  // label_file = fopen(str_labels_filepath.c_str(), "r");
  auto label_file = readfile_vfs_char(str_labels_filepath);
  std::string label_file_str(label_file.begin(), label_file.end());
  std::istringstream label_file_stream(label_file_str.c_str());
  // if (!label_file)
  // {
  ...
  for (int i = 0; i < NUM_SAMPLES; i ++ )
  {
    ...
    // labels[i] = tmp;
    std::string tmp;
    std::getline(label_file_stream, tmp);
    labels[i] = std::stoi(tmp);
