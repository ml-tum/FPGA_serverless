...
Addresses parse_cmdline_for_net_config()
{
  ip4::Addr address{OS::get_cli_ip4()};
  ip4::Addr netmask{OS::get_cli_nmsk()};
  ip4::Addr gateway{OS::get_cli_gw()};
  ip4::Addr dns = gateway;
...
void configure(const rapidjson::Value& net)
{
  ...
  for(auto& val : configs)
  {
    ...
    {
      config_stack(stack, parse_cmdline_for_net_config());
    }
  }
