...
int wmain(uint64_t size, std::string_view input) {
	std::cout << "Size = " << size << std::endl;
	std::cout << "--- Begin on input ---" << std::endl;
...
http::Response handle_request(const http::Request& req)
{
  ...
  {
    wmain(margc, std::string_view(margv[margc-1]));
  ...
  // POST /
  else if(req.method() == http::POST && req.uri().to_string() == "/")
  {
    wmain(req.body().length(), req.body());
    std::stringstream resp_body;
    for (std::string tmp_out: func_out) {
      resp_body << tmp_out;
    }
...
int Service::start(const std::string& cmd)
{
  ...
  [] (net::tcp::Connection_ptr conn) {
    ...
    [conn] (auto buf)
    {
      ...
      try
      {
        const std::string read_data((const char*) buf->data(), buf->size());
        // try to parse the request
	if (tot_buf.empty()) {
          http::Request req{read_data};

	  if(req.header().has_field("Transfer-Encoding") && req.header().value("Transfer-Encoding")=="chunked") {
	    tot_buf = tot_buf.append(read_data);
	  } else {
	    printf("Not chunked\n");
	  ...
	  //printf("%c\n", read_data[read_data.length() - 5 ]);
	  if (read_data[read_data.length() - 5 ] == '0') {
	  //if (read_data[0] == '0') {
	    //printf("Na teleiwse\n");
            http::Request req{tot_buf};
            auto res = handle_request(req);
            conn->write(res);
	    tot_buf.clear();
	  }
