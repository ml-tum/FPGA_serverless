...
namespace rosetta
{
  ...
  int CLWorld::addProgram(std::string filename)
  {
    ...
    auto bsfile = readfile_vfs(filename);
    size_t code_size = bsfile.size();
    kernel_code = (char*) bsfile.data();
    ...
    /* When program is created by clCreateProgramWithBinary, clBuildProgram() can be skipped */
