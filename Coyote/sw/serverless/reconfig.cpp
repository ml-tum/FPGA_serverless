...
void Reconfig::reconfigure(void *vaddr, uint32_t len)
{
    ...
    Printer(vfid)<<"Reconfiguration completed, addr="<<std::hex<<vaddr<<", len="<<std::dec<<len
        <<", duration="<<std::chrono::duration_cast<std::chrono::milliseconds>(duration).count()<<" ms"
        <<endl;
}
 ...
 * @param oid - operator ID
 */
void Reconfig::addBitstream(std::string name, int32_t oid)
{
    if (bstreams.find(oid) == bstreams.end())
    {
        // Stream
